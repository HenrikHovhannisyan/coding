  ...
  return (
    ...
    >
      ...
      <p>
        ...
        переменных, использующие <code>var</code>, поднимаются/hoisted в верхнюю
        часть их функциональной/локальной области видимости (если объявляется
        внутри функции) или в глобальную область видимости (если объявляется вне
        функции) независимо от того, где была сделана фактическая декларация.
      </p>
      <p>
        Объявления переменных и функций помещаются в память на этапе компиляции,
        но они остаются именно там, где мы ввели их в свой код.
      </p>
      <p>
      ...
      <p>
        ...
        Это позволяет нам использовать <strong>функцию</strong> до того, как мы
        объявим её в своем коде. Например:
      </p>
      <CodeExample
...
var name;`}
      ...
      <p>
        ...
        <code>const</code> и <code>let</code>, и, как правило, они рекомендуются
        во избежание неожиданных осложнений при подъеме.
      </p>
...
name = "Alex";`}
      ...
      <ul className="list-disc ms-4">
        <li>
          <p>
            <code>const</code> и <code>let</code> имеют область видимости блока.
          </p>
        </li>
      </ul>
      <p>
      ...
      <p>
        ...
        переменную в коде над объявлением не является ошибкой, если этот код не
        выполняется (например, тело функции или просто мертвый код), но ошибка
        будет выдана, если мы запросим доступ к переменной до её инициализации.
      </p>
      <p>
      ...
      <p>
        ...
        инициализированы без значения, в то время как <code>const</code> выдаст
        ошибку <code>ReferenceError</code>, если ты попытаешься объявить её без
        одновременного присвоения ей значения. Так что{" "}
...
const c = 5;`}
      ...
      <p>
        Однако, если ты напишешь <code>let newName = 'Alex';</code> это не будет
        доступно в глобальном объекте <code>window</code> - следовательно, ты не
