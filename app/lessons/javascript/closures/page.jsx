...
https://vkontakte.com`}
      />
      <h2 className="text-2xl font-bold mb-2">Лексическая область видимости</h2>

      <p>
        <code>init()</code> создаёт локальную переменную <code>name</code> и
        определяет функцию
        <code>displayName()</code>. <code>displayName()</code> — это внутренняя
        функция — она определена внутри <code>init()</code> и доступна только
        внутри тела функции <code>init()</code>. Обратите внимание, что функция{" "}
        <code>displayName()</code> не имеет никаких собственных локальных
        переменных. Однако, поскольку внутренние функции имеют доступ к
        переменным внешних функций, <code>displayName()</code> может иметь
        доступ к переменной <code>name</code>, объявленной в родительской
        функции <code>init()</code>.
      </p>
      <CodeExample
        code={`function init() {
...
init();`}
      ...
      <p>
        ...
        действия переменной определяется по её расположению в коде (это очевидно
        лексически), и вложенные функции имеют доступ к переменным, объявленным
        вовне. Этот механизм и называется Lexical scoping (область действия,
...
myFunc();`}
      ...
      <p>
        ...
        <code>displayName()</code> была возвращена из внешней до того, как была
        выполнена.
      </p>
      <p>
      ...
      <p>
        ...
        <code>makeFunc</code>. Экземпляр функции <code>displayName</code> в свою
        очередь сохраняет ссылку на своё лексическое окружение, в котором есть
        переменная <code>name</code>. По этой причине, когда происходит вызов
        функции <code>myFunc</code>, переменная <code>name</code> остаётся
        доступной для использования и сохранённый в ней текст "Mozilla"
        передаётся в <code>console.log</code>.
      </p>
      <p>
...
12`}
      />
      <p>
        Здесь мы определили функцию <code>makeAdder(x)</code>, которая получает
        единственный аргумент <code>x</code> и возвращает новую функцию. Эта
        функция получает единственный аргумент <code>y</code> и возвращает сумму{" "}
